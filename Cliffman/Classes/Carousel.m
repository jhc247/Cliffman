...
@implementation Carousel {
    ...
    CCSprite* _topArrow;
    CCSprite* _bottomArrow;
    
    int totalHelmets;
}

+ (Carousel*) createCarousel: (CGPoint)position vertical:(BOOL)vertical width:(float)width height:(float)height numElements:(int)numElements elements:(NSArray*)elements worldNum:(int)worldNum topArrow:(CCSprite*)topArrow bottomArrow:(CCSprite*)bottomArrow {
    return [[self alloc] init:position vertical:vertical width:width height:height numElements:numElements elements:elements worldNum:worldNum topArrow:topArrow bottomArrow:bottomArrow];
}

- (id) init: (CGPoint)position vertical:(BOOL)vertical width:(float)width height:(float)height numElements:(int)numElements elements:(NSArray*)elements worldNum:(int)worldNum topArrow:(CCSprite*)topArrow bottomArrow:(CCSprite*)bottomArrow {
...
- (void)update:(CCTime)delta {
    ...
    if (_vertical) {
        _topArrow.opacity = (self.position.y > _maxHeight + buffer) ? 0.7 : 0;
        _bottomArrow.opacity = ((self.position.y - _height) < _minHeight - buffer) ? 0.7 : 0;
        ...
        _topArrow.opacity = (self.position.x < _minWidth - buffer) ? 0.7 : 0;
        _bottomArrow.opacity = (self.position.x + _width > _maxWidth + buffer) ? 0.7 : 0;
    }
    /*
    if (_vertical) {
