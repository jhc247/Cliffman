...
- (id)init: (CGPoint)position shoot:(BOOL)shoot{
    ...
    self.anchorPoint = ccp(0.55, 0.45);
    self.physicsBody = [CCPhysicsBody bodyWithRect:(CGRect){ccp(self.contentSize.width*.40,self.contentSize.height*.15), CGSizeMake(self.contentSize.width*.3, self.contentSize.height*.6)} cornerRadius:0];
...
- (void)update:(CCTime)delta {
    ...
    if (currentSpear != NULL && currentSpear.state != Detaching && self.state != Dying && self.state != Won) {
        ...
        float length = ccpDistance(orig,currentSpear.position);
        float angle = [Spear getAngle:orig target:currentSpear.position];
        float ropeThickness = [CCDirector is_iPad] ? ROPE_THICKNESS : ROPE_THICKNESS / IPAD_TO_IPHONE_HEIGHT_RATIO;
    ...
    if (self.state == Rising && ticks > 2) {
        ...
        float angle1 = self.flipX ? self.rotation : -self.rotation;
        
        float deltaX = sinf(CC_DEGREES_TO_RADIANS(angle1)) * self.contentSize.height*.15;
        float deltaY = cosf(CC_DEGREES_TO_RADIANS(angle1)) * self.contentSize.height*.15;
        
        float orig_x = self.flipX ? self.position.x + deltaX : self.position.x - deltaX;
        float orig_y = self.position.y + deltaY;
        CGPoint orig = ccp(orig_x, orig_y);
        
        float ropeLength = ccpDistance(orig, currentSpear.position);
        float ropeMax = [CCDirector is_iPad] ? ROPE_MINMAX_LENGTH : ROPE_MINMAX_LENGTH / IPAD_TO_IPHONE_HEIGHT_RATIO;
        ropeLength = (ropeLength >= ropeMax / 2) ? ropeLength : ropeMax / 2;
        //ropeLength *= 0.9;
        ...
        if (currentSpear.state == Attached) {
            [CCPhysicsJoint connectedDistanceJointWithBodyA:self.physicsBody bodyB:currentSpear.physicsBody anchorA:ccp(self.contentSize.width*.55, self.contentSize.height*.60) anchorB:ccp(currentSpear.contentSize.width*.5,currentSpear.contentSize.height*.17) minDistance:0 maxDistance:ropeLength];
        }
        [_jointLock unlock];
...
- (void) killPlayer {
    ...
    [self stopAllActions];
    [self removeSpear];
    self.rotation = self.flipX ? self.rotation + 90: self.rotation - 90;
